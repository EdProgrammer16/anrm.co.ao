...
new Chart(ctx1, {
  ...
  data: {
    ...
    datasets: [{
      label: "Concessões / Recurso Mineiro",
      weight: 9,
      cutout: 30,
      tension: 0.9,
      pointRadius: 2,
  ...
  options: {
    ...
    new Chart(ctx1, {
      type: "pie",
      data: {
        labels: ['Área Provincial', 'Área Total das Concessões'],
        datasets: [{
          label: "Área Ocupada",
          weight: 9,
          cutout: 0,
          ...
          data: [
            <?php
              ...
              foreach($params['provincias'] as $key){
                if( strpos($key['pro_titulo'], $params['proNome']) !== false ) {
                  $area_prov = $key['pro_area'];
                  break;
                }
              }
              echo $area_prov;
  ...
  $(document).ready(function(){
    $('.coluna').click(function(){
      ...
      if(endpoint == 'concessao'){
        ...
        document.location.href = '<?= DIRPAGE; ?>dashboard/provincia/'+'<?= base64_encode($params['proNome']); ?>/'+endpoint+'/'+search;
      }
    })
  })
</script>
